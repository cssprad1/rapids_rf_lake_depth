...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        for k, (train, test) in enumerate(k_fold.split(covariates_train, labels_train)):
            ...
            y_dask_cudf_test = dask_cudf.from_cudf(labels_train.iloc[test], npartitions=n_workers)
            X_dask_cudf_test, y_dask_cudf_test = dask_utils.persist_across_workers(c,
                                                  [X_dask_cudf_test,
                                                   y_dask_cudf_test])
            depth_rf_model.fit(X_dask_cudf, y_dask_cudf)
            wait(depth_rf_model.rfs)
            predictions = depth_rf_model.predict(X_dask_cudf_test).compute().to_array()
            #score = mean_absolute_error(labels_train.iloc[test], predictions)
            scores.append(0)
            et = time.time()
