...
class Aviris():
    ...
    def __init__(self):
        ...
        self.DATASET_PATH = os.path.join(os.getcwd(), 'load_dataset',
                                         'LakeDepth',
                                         'aviris_bands_extract_final.csv')

        self._RANDOM_STATE = 42

        print(" - from DATA: reading csv into GPU memory")
        self.full_dataset = cudf.read_csv(self.DATASET_PATH)
        ...
        self.covariates_train, self.covariates_test, \
            self.labels_train, self.labels_test = train_test_split(self.covariates,
                                                                   self.labels,
                                                                   test_size=Aviris.INIT_TEST_SIZE,
                                                                   shuffle=True,
    ...
    def nth_row_sampling(self, start_index, nth_band):
        self.covariates = self.covariates.iloc[:, start_index::nth_band]
