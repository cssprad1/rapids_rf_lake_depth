...
for i in range(0, 1):
    ...
    cdf_raster = pd.DataFrame(img_nd_array)
    n_rows_raster, n_cols_raster = cdf_raster.shape
    print(" - loading model")
    model_load = crf.load_raw_model('sklearn_model_0.sav')
    
    index_0 = n_rows_raster//2
    index_1 = index_0 * 2
    index_3 = n_rows_raster % 2 # Any indeces left out?
    print(" - starting predictions")
    predictions_1 = model_load.predict(cdf_raster[:index_0]) # Predict first half
    predictions_2 = model_load.predict(cdf_raster[index_0:]) # Predict other hals
    print(" - ending predictions")
    concat_predictions = pd.concat([predictions_1, predictions_2], axis=0)
    ...
    file_name_predicted = file_name_no_extension + '_predicted_SKLEARN_0.tif'
